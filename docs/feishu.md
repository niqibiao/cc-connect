...
| 权限名称 | 权限标识 | 用途 |
|---------|---------|------|
| 获取与更新用户基本信息 | `contact:user.base:readonly` | 获取用户真实姓名（群聊消息记录显示用） |
...
cc-connect 支持在群聊中自动记录所有消息，并通过 AI Agent 进行智能总结。

### 前置配置

要使用群聊总结功能，需要确保飞书应用具备以下配置：

1. **接收全部群消息**：进入飞书开放平台 → 应用详情 → 左侧「事件订阅」→ 确认已添加 `im.message.receive_v1` 事件。然后进入「机器人」能力设置 → 将消息接收模式从「只接收@机器人的消息」改为「**接收群内所有消息**」。

2. **通讯录权限**（用于显示真实用户名）：进入「权限管理」→ 确认已开通 `contact:user.base:readonly`（获取与更新用户基本信息）。如果没有此权限，消息记录中的用户名将显示为 open_id。

### 使用方式

配置完成后，群聊中**所有成员发送的消息**（无论是否 @bot）都会被自动记录。

在群聊中 @bot 并用自然语言请求总结即可，例如：

```
@bot 总结一下群聊
@bot summarize the chat
@bot 大家刚才聊了什么
```

Agent 会自动调用 `cc-connect chatlog` 获取聊天记录，生成摘要后回复。总结完成后，Agent 会询问你是否需要清空已读的历史消息。

> 💡 **说明**：
> - 消息记录存储在内存中，重启 cc-connect 后会清空
> - 每个群聊的消息记录独立存储，互不影响
> - 每个群聊最多保留 500 条消息（环形缓冲区）
> - 仅记录文本和富文本消息，图片/语音等暂不记录
